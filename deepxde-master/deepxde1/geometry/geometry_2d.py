...
class RectangleClusteredMHD(Geometry):
    """
    Args:
        xmin: Coordinate of bottom left corner.
        xmax: Coordinate of top right corner.
    ...
    def __init__(self, xmin, xmax):
        ...
        self.xmin, self.xmax = np.array(xmin), np.array(xmax)
        super(RectangleClusteredMHD, self).__init__(
            len(xmin), (self.xmin, self.xmax), np.linalg.norm(self.xmax - self.xmin)
        )
        self.perimeter = 2 * np.sum(self.xmax - self.xmin)
        self.area = np.prod(self.xmax - self.xmin)
...
#from here
    ...
    def random_points(self, n, random="pseudo"):
        ...
        x1 = ([0.2,0.2])*np.random.rand(900,2)+[-0.1,0]
        for i in range(900):
            x1[i,0]=(1+30*x1[i,1])*x1[i,0]+x1[i,1]
        
        x2 = ([2.0,0.2])*np.random.rand(100,2)+[-1.0,0]
...
#to here is a copy from geometry_nd.py
    ...
    def random_boundary_points(self, n, random="pseudo"):
        ...
        x2=[]
        for i in range(100):
            x2.append([0.2*np.random.rand()-0.1,0])
        x2=np.array(x2)
        x=np.vstack((x1,x2))
        return x
