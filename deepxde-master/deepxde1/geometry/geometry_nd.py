...
class Hypercube(Geometry):
    ...
    def random_boundary_points(self, n, random="pseudo"):
        ...
        pts = [x_corner]
        nopoints = n / 8
        cube = Cuboid(self.xmin[:-1], self.xmax[:-1])
        for z in [self.xmin[-1], self.xmax[-1]]:
            u = cube.random_points(int(np.ceil(nopoints)), random=random)
            pts.append(np.hstack((u, np.full((len(u), 1), z))))
            
        cube = Cuboid([self.xmin[0],self.xmin[1],self.xmin[3]], [self.xmax[0], self.xmax[1], self.xmax[3]])
        for y in [self.xmin[2], self.xmax[2]]:
            u = cube.random_points(int(np.ceil(nopoints)), random=random)
            pts.append(np.hstack((u[:, 0:2], np.full((len(u), 1), y), u[:, 2:])))
            
        cube = Cuboid([self.xmin[0],self.xmin[2],self.xmin[3]], [self.xmax[0], self.xmax[2], self.xmax[3]])
        for x in [self.xmin[1], self.xmax[1]]:
            u = cube.random_points(int(np.ceil(nopoints)), random=random)
            pts.append(np.hstack((u[:, 0:1], np.full((len(u), 1), x), u[:, 1:])))
            
        cube = Cuboid(self.xmin[1:], self.xmax[1:])
        for p in [self.xmin[0], self.xmax[0]]:
            u = cube.random_points(int(np.ceil(nopoints)), random=random)
            pts.append(np.hstack((np.full((len(u), 1), p), u)))
