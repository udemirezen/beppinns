...
ic1l = dde.NormalDisWeightDirichletBC(geom, lambda x: 1.4*np.ones((len(x),1)), boundary_time_left, component=0)
ic2l = dde.NormalDisWeightDirichletBC(geom, lambda x: 0.1*np.ones((len(x),1)), boundary_time_left, component=1)
ic3l = dde.NormalDisWeightDirichletBC(geom, lambda x: 1.79071429*np.ones((len(x),1)), boundary_time_left, component=2)

ic1r = dde.NormalDisWeightDirichletBC(geom, lambda x: 1.0*np.ones((len(x),1)), boundary_time_right, component=0)
ic2r = dde.NormalDisWeightDirichletBC(geom, lambda x: 0.1*np.ones((len(x),1)), boundary_time_right, component=1)
ic3r = dde.NormalDisWeightDirichletBC(geom, lambda x: 2.505*np.ones((len(x),1)), boundary_time_right, component=2)


bic = [bc1l, bc2l, bc3l, bc1r, bc2r, bc3r, ic1l, ic2l, ic3l, ic1r, ic2r, ic3r]

data = dde.data.PDE(geom, 3, mhd, bic, 1008, 640)
