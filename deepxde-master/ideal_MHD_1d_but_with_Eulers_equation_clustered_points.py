...
geom = dde.geometry.RectangleClusteredEuler([0,0], [1,1.99])
...
ic1l = dde.DirichletBC(geom, lambda x: 1.4*np.ones((len(x),1)), boundary_time_left, component=0)
ic2l = dde.DirichletBC(geom, lambda x: 0.1*np.ones((len(x),1)), boundary_time_left, component=1)
ic3l = dde.DirichletBC(geom, lambda x: 1.79071429*np.ones((len(x),1)), boundary_time_left, component=2)

ic1r = dde.DirichletBC(geom, lambda x: 1.0*np.ones((len(x),1)), boundary_time_right, component=0)
ic2r = dde.DirichletBC(geom, lambda x: 0.1*np.ones((len(x),1)), boundary_time_right, component=1)
ic3r = dde.DirichletBC(geom, lambda x: 2.505*np.ones((len(x),1)), boundary_time_right, component=2)
